...
/**
 * TDSQL数据库适配器 (腾讯云分布式数据库)
 * TDSQL兼容MySQL协议,使用MySQL驱动连接
 * 
 * @author SchemaSync Team
 * @since 2026-04-26
 */
@Component
public class TDSQLAdapter implements DatabaseAdapter {

    private static final Logger log = LoggerFactory.getLogger(TDSQLAdapter.class);
    ...
    public String getDatabaseVersion(Connection conn) throws SQLException {
        try (Statement stmt = conn.createStatement();
            ...
            if (rs.next()) {
                return "TDSQL " + rs.getString(1);
            }
        }
        return "TDSQL Unknown";
    }
    ...
    public Connection connect(DataSourceConfig config) throws SQLException {
        ...
        hikariConfig.setJdbcUrl(String.format(
            "jdbc:mysql://%s:%d/%s?useUnicode=true&characterEncoding=utf8&useSSL=true&serverTimezone=Asia/Shanghai",
            config.getHost(), 
            config.getPort(), 
            config.getDatabase()
        ));
    ...
    public SchemaDictionary exportSchema(DataSourceConfig config, ExportOptions options) throws SQLException {
        ...
        metadata.setDatabaseType("TDSQL");
