...
- 前端页面 GenerateView.vue
  - 表单字段：数据字典文件上传、数据库类型下拉（mysql、gaussdb_mysql、gaussdb_oracle）
  - 动作：生成预览、下载SQL
...
- 服务层 DdlGeneratorService.java
  ...
  - 根据 databaseType 路由到不同生成策略（MySQL风格、GaussDB Oracle风格）
...
- 表单与状态
  ...
  - generating：生成中标志
  - ddlScript：生成的DDL文本
  - form.databaseType：当前选择的数据库类型（默认 mysql）
- 交互流程
...
图表来源
- [GenerateView.vue:71-104](file://schemasync-frontend/src/views/GenerateView.vue#L71-L104)
...
- 参数约定
  ...
  - databaseType：mysql/gaussdb_mysql/gaussdb_oracle（默认 mysql）
- 错误处理
...
图表来源
- [DdlGeneratorService.java:40-97](file://schemasync-backend/src/main/java/com/schemasync/service/DdlGeneratorService.java#L40-L97)
- [DdlGeneratorService.java:109-141](file://schemasync-backend/src/main/java/com/schemasync/service/DdlGeneratorService.java#L109-L141)
- [DdlGeneratorService.java:146-177](file://schemasync-backend/src/main/java/com/schemasync/service/DdlGeneratorService.java#L146-L177)
...
- 快速修复
  - 重新上传正确格式的数据字典文件
  - 切换数据库类型重试（例如从 gaussdb_oracle 切换到 mysql）
...
## 结论
GenerateView 提供了简洁的全量DDL生成入口，支持多数据库类型选择与预览/下载。后端具备完善的解析与生成逻辑，能针对不同数据库风格输出相应DDL。当前页面在用户体验方面仍有提升空间，包括代码高亮、语法检查、行号、复制、国际化等。后续可结合 GenerationOptions 与 DDLGenerator 接口，逐步完善差异化DDL生成与执行模拟能力。
...
- 打开“全量DDL脚本生成”页面
- 选择数据字典文件
- 选择目标数据库类型（MySQL/GaussDB MySQL兼容/GaussDB Oracle兼容）
- 点击“生成DDL脚本”，在预览区查看结果
- 如需保存，点击“下载SQL文件”
